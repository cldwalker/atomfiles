...
let varCount = 1
// Evals block to new r# var where # is incremented by one each time
atom.commands.add('atom-text-editor', 'me:eval-block-and-def-new-result', () => {
  const editor = atom.workspace.getActiveTextEditor()
  const sexp = editor.getTextInBufferRange(EditorUtils.getCursorInBlockRange(editor))
  const code = `(def r${varCount} ${sexp})`
  printAndEval(code)
  varCount += 1
})

// Evals block to same r0 var
atom.commands.add('atom-text-editor', 'me:eval-block-and-def-same-result', () => {
  const editor = atom.workspace.getActiveTextEditor()
  const sexp = editor.getTextInBufferRange(EditorUtils.getCursorInBlockRange(editor))
  const code = `(def r0 ${sexp})`
  printAndEval(code)
})
